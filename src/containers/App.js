...
class App extends Component {
    ...
    saveDataHandler = () => {
        localStorage.setItem(
            ...
            JSON.stringify(this.state.moods)
        );
    };

    populateStateFromLocalStorage = () => {
        const retrievedData = JSON.parse(
            localStorage.getItem(this.state.currentDate)
        );
        if (retrievedData) this.setState({ moods: retrievedData });
        else this.setState({ moods: this.defaultMoods });
    };
    ...
    render() {
        return (
            <div>
                ...
                />
                <Confirmation saveData={this.saveDataHandler} />
            </div>
        );
    }
}
